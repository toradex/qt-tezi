                ...
                if (results) {
                    for (const typename AdapterType::ObjectMember m : object) {
                        if (propertiesMatched.find(m.first) != propertiesMatched.end())
                            continue;
                        results->pushError(context, "Property '" + m.first +
                                "' could not be validated using 'properties' constraint.");
                    }
                }
        ...
        for (const typename AdapterType::ObjectMember m : object) {
            if (propertiesMatched.find(m.first) == propertiesMatched.end()) {
                ...
                if (!validator.validateSchema(*additionalPropertiesSubschema)) {
                    if (results) {
                        results->pushError(context, "Failed to validate '" + m.first +
                                "' against 'additionalProperties' constraint.");
                    }
